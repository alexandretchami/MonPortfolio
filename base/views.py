...
def projectPage(request, pk):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            comment.save()
            messages.success(request, "Votre commentaire a bien été envoyé.")
...
def addSkill(request):
    ...
    if request.method == "POST":
        ...
        form.save()
        messages.success(request, "Votre compétence a été ajoutée avec succès.")
        return redirect("home")
    context = {"form": form}
...
def addEndorsement(request):
    ...
    if request.method == "POST":
        ...
        form.save()
        messages.success(
            request, "Merci, votre récommandation a été ajoutée avec succès."
        )
        return redirect("home")
    context = {"form": form}
